...
| Field | Value |
|---|---|
| Current version | **3.7.15** (`VERSION`) |
...
|---|---|
| `.tcyr` test files | 54 |
| Total assertions | **1475**, 0 failures (1468 released at 3.7.15 + **7 unreleased** EC-inversion KATs in the working tree) |
...
- **Unreleased (working tree)**: +7 (P-256 verify inversion speedup —
  ...
  clean A/B on 6.2.12. Not yet versioned — awaiting Robert's version assignment.)
...
| 3.7.x — EC scalar-mult ≤ 10 ms squeeze | **in progress** (lever 1 done, unreleased) | Comb-G + windowed-Q shipped 3.7.8. **Lever 1 — inversion addition-chains — DONE (unreleased)**: field `fp_p256_inv` → fixed `2^k-1`-block chain, scalar `fn_p256_inv` → 4-bit window; `ecdsa_p256_verify` 12.50 → **11.37 ms** (clean A/B, ~9%). **≤ 10 ms still open** — next levers: mixed Jacobian+affine addition → optional Karatsuba `u256_mul_full` (rising risk; the last touches the audited shared multiply → security re-review). |
