...
| Field | Value |
|---|---|
| Current version | **3.7.16** (`VERSION`) |
...
|---|---|
| `.tcyr` test files | 54 |
| Total assertions | **1478**, 0 failures (1468 at 3.7.15 + **10 at 3.7.16**: 7 inversion-chain + 3 mixed-add EC-squeeze KATs) |
...
| 3.7.x — EC scalar-mult ≤ 10 ms squeeze | **in progress** (levers 1 + 2a shipped 3.7.16; 2b + 3 → 3.7.17) | Comb-G + windowed-Q shipped 3.7.8. **Lever 1 — inversion addition-chains — DONE**: field `fp_p256_inv` → fixed `2^k-1` chain, scalar `fn_p256_inv` → 4-bit window; 12.50 → **11.37 ms**. **Lever 2a — mixed add on the affine comb-G — DONE**: `pt_add_mixed` (madd-2007-bl) on the affine `u1·G` table; isolated A/B 11.32 → **11.00 ms**. **≤ 10 ms still open** (now ~11.0 ms) — verify is doubling/inversion-bound, so mixed-add is capped; remaining: **lever 2b** (`u2·Q`-window adds → affine via per-verify batch inversion, ~0.3–0.5 ms) and **lever 3** Karatsuba `u256_mul_full` (touches the audited shared multiply → security re-review; the one that actually crosses 10 ms). |
