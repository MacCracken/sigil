...
| **ADR 0006 ≤ 10 ms — DECISION PENDING** | **open — needs Robert** | `ecdsa_p256_verify` measured **9.732 ms** at 3.12.2, below the target ADR 0006 parked as "not reachable". The ADR was deliberately **not** closed by the measurement: the crossing is narrow (~7 %) and single-host, and the disposition was Robert's. Open question: declare it met, re-measure on a second host first, or set a new target? See [`roadmap.md`](roadmap.md). |
...
needs a decision from Robert rather than engineering work.** No open
