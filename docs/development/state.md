...
**How they receive sigil (checked at 3.13.2):** none is on 3.13.x. `lib/sigil.cyr` comes
from cyrius's stdlib snapshot, which wins over a same-named `[deps.sigil]` (the 6.6.6
snapshot carries 3.12.18, 6.6.2's carries 3.12.16), so a sigil release reaches them when
cyrius re-vendors it. phylax and argonaut also take sigil pieces through libro, which pins
its own sigil (libro 2.10.0 → 3.12.9, 2.10.3 → 3.12.18). ark, aegis, phylax, mela and stiva
pin cyrius 6.6.2, below the 6.6.6 that sigil 3.13.x needs. A downstream check therefore
has to overlay the new bundle on a scratch `CYRIUS_HOME` and compare at one toolchain; the
3.13.2 run is in the CHANGELOG `[3.13.2]`.

## Recently shipped

| Version | Date | Headline |
|---|---|---|
| 3.13.2 | 2026-09-25 | **Closeout pass before 3.14.0.** The benchmark 3.13.1 shipped without, as an interleaved A/B against 3.13.0: `sha512_4kb` +16–19% (the L19 schedule wipe), the rest near noise, and the 4–7% gap to the `v3.13.0-closeout` rows is the host. New `scripts/state-sync.sh` keeps this file's derivable fields in step (CI runs `--check`); CI and `check.sh` run `cyrius deps --verify`. One unreferenced private function removed; the 4.0 constructor removal is tracked in the roadmap again. No behaviour change. |
| 3.13.1 | 2026-09-23 | **Cleanup of the issues filed at 3.13.0.** X.509 chain-link race (a forged link verified under concurrency) fixed with a per-call digest; stored `TRUST_REVOKED` no longer lifted by a valid signature; `certpin_compute_spki_pin` computed in-process (was a fail-open openssl pipeline); ECDSA sign ladder blinded (one-bit residual); Ed25519 small-order keys refused; canonical-DER / exact-length quote parsing; atomic, checked trust-store and keyring saves; per-call JSON parser state; CAS-guarded AES / BLAKE2b / SHA-512 / Ed25519 inits. Breaking: see the CHANGELOG. |
| 3.13.0 | 2026-09-23 | **Toolchain 6.6.6 + known-issue repairs.** Pin 6.6.4 → 6.6.6, deps refreshed. LUKS keyfile no longer left in `/tmp` — cyrius skips a `defer` on a value-form Result return, so all 8 `defer`s were removed and every exit releases explicitly (`fd_hygiene.tcyr`). No raw syscalls in any tree; numeric open flags → per-target `O_*`. 429 public fns documented (6.6.6 doc gate). Security fixes and one breaking change (`keyring_validate_chain` needs issuer signatures) — see the CHANGELOG. AES-GCM 11× faster; `ed25519_sign` 1.84× slower (public-key re-derivation). |
...
| Slot | State | Notes |
|---|---|---|
| **3.14.0 — retire `cbank()`** | **SCHEDULED, not started** (moved from 3.13.0 at 3.13.0) | The follow-on to 3.12.9's asymmetric de-banking: localise the remaining **symmetric + EC** scratch (**188 `cbank()` sites across 28 files** — sha2/sha_ni/blake2b, hmac/hkdf/tls12_prf/argon2, aes-gcm/chacha20/poly1305, ecdsa p256+p384+sign, ed25519/x25519/bigint_ext, authenticode/trust/verify) and then delete `src/crypto_scratch.cyr`. Removes the 63-lifetime-thread ceiling library-wide, retires the `crypto_banks_exhausted()` / `crypto_banks_claimed()` API added at 3.12.9 (**an API removal — hence a minor**), and reclaims most of the remaining 785,408 B of static data (every banked global costs **8×** its declared size — [note 003](../architecture/003-global-arrays-are-eight-bytes-per-element.md)). **Not a vulnerability**: a symmetric/EC lane collision is a corrupted digest or failed handshake, fail-closed, not a forged accept. Method is fixed — bites per module, each mutation-proven. The CLAUDE.md closeout pass that must precede the minor ran at 3.13.2. Plan: [`roadmap.md`](roadmap.md#planned--3140--retire-cbank). |
| 3.12.x — RSA de-banking | **CLOSED at 3.12.9** | 3.12.3–3.12.6 took the RSA/PSS **verify** surface off shared lanes after two authentication bypasses; **3.12.9** finished the **sign** path (23 globals) and the bignum engine (17), closing and archiving the 2026-08-08 forged-signature issue — `docs/development/issues/` is **clear again (0 open, 17 archived)**. Static data 10,779,648 → **785,408 B (−92.7%)**. Audit `2026-08-14-3.12.9-rsa-sign-debanking-audit.md`. |
...
**Two open in-flight items: the 3.14.0 `cbank()` retirement (scheduled
engineering work, not started) and the ADR 0006 ≤ 10 ms disposition, which
needs a decision rather than engineering.** The 3.12.x cycle **closed** at 3.12.2; the 3.6.x
...
**Carried-forward roadmap Backlog (genuinely open — do NOT bury), as of 3.13.2:**
the ADR 0006 ≤ 10 ms disposition (now *met*, awaiting a decision); retire-sysinfo;
scatter-store (parked, moot); CLMUL-GHASH (re-scope); ML-KEM-768; the
`#derive(Serialize)` switch (two hand-written serializers); the 4.0 removal of the
heap-allocating trust constructors; the Windows `cass` runs (the 3.7.15 entropy
acceptance and the three 6.6.6-pin checks) and the mabda / yukti re-vendor; the UEFI
Secure Boot firmware-interop gate; the honest 3.10/3.11 gap in `benches/history.csv`;
a possible full `asm{}` CIOS inner loop; the cyrlint line-length carve-out (waits on a
per-line allow); the undecided catch-up audit for 3.12.3–3.12.7; 3.12.7's
unreproducible assertion count; the cyrfmt size cap on `dist/sigil.cyr` (upstream);
and the one open issue, L23 (needs an IMA host). (Until 3.13.2 this list still named
the TDX/SGX in-quote PCK chain walk, found already shipped at 3.8.0, and
retire-bank-indexing, folded into 3.14.0.) **Note the gate correction:** several of these were recorded
...
could skip. See [`roadmap.md`](roadmap.md) "Outstanding work" for the
authoritative list.
...
dev host (Linux 7.2.6-arch2-1, x86_64). Cross-host smoke
wrapper is not yet scaffolded — when AGNOS gains a cross-host
CI fleet, list the hosts here.

## Audit floor

**One LOW open since 3.13.1:** L23 from the 3.13.0 list — `ima_get_status`'s
`policy_loaded` may read backwards on kernels built without
`CONFIG_IMA_WRITE_POLICY`. It needs a host with IMA enabled before the logic
changes (`docs/development/issues/2026-09-23-ima-policy-loaded-needs-ima-host.md`);
the dev host's kernel has `CONFIG_IMA` unset. Before that the floor was **empty —
cleared at 3.7.3, held through 3.12.9.** **3.12.2's audit
...
Outstanding: one LOW (L23, above). Zero CRITICAL / HIGH / MEDIUM.
