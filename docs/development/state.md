...
| Field | Value |
|---|---|
| Current version | **3.13.1** (`VERSION`) |
...
| Slot | State | Notes |
|---|---|---|
| **3.13.0 — retire `cbank()`** | **SCHEDULED, not started** | The follow-on to 3.12.9's asymmetric de-banking: localise the remaining **symmetric + EC** scratch (**188 `cbank()` sites across 28 files** — sha2/sha_ni/blake2b, hmac/hkdf/tls12_prf/argon2, aes-gcm/chacha20/poly1305, ecdsa p256+p384+sign, ed25519/x25519/bigint_ext, authenticode/trust/verify) and then delete `src/crypto_scratch.cyr`. Removes the 63-lifetime-thread ceiling library-wide, retires the `crypto_banks_exhausted()` / `crypto_banks_claimed()` API added at 3.12.9 (**an API removal — hence a minor**), and reclaims most of the remaining 785,408 B of static data (every banked global costs **8×** its declared size — [note 003](../architecture/003-global-arrays-are-eight-bytes-per-element.md)). **Not a vulnerability**: a symmetric/EC lane collision is a corrupted digest or failed handshake, fail-closed, not a forged accept. Method is fixed — bites per module, each mutation-proven, per CLAUDE.md closeout the last 3.12.x patch runs the closeout pass first. Plan: [`roadmap.md`](roadmap.md#planned--3130--retire-cbank). |
...
**Two open in-flight items: the 3.13.0 `cbank()` retirement (scheduled
