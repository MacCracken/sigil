...
- [0008 — A native asm multiply, and a deliberately non-constant-time modexp](0008-native-asm-multiply-and-public-modexp.md) — **Accepted**: own the 64×64→128 multiply in `asm{}` (no CPUID probe — `MUL r/m64` is baseline x86-64), ship `bn_mont_modexp_pub` with an explicit "public exponents only" rule, and record that ADR 0006's "asm multiply is gated on upstream cyrius" premise was too pessimistic. `ecdsa_p256_verify` now measures below ADR 0006's parked ≤ 10 ms target — that disposition is left open, deliberately, rather than closed as a side-effect.
