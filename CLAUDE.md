...
- **No raw syscalls (added 3.13.0).** Every syscall goes through the stdlib's per-target helpers — the `sys_*` wrappers, and `lib/io.cyr`'s `file_*` / `x*` set (`file_open`, `xopen`, `xunlink`, …) for file operations — never `syscall(N, …)` with a hand-spelled number, and never a bare `sys_open` on a path agnos can reach (agnos's is `(name, namelen, flags)`). Open flags are per-target `O_*` names, never numeric literals: `577` is `O_WRONLY|O_CREAT|O_TRUNC` only on x86_64 Linux, and on Darwin it silently drops `O_TRUNC`. This covers `tests/`, `fuzz/`, benches and `programs/` too. **One tracked exception:** the `syscall(SYS_FCNTL, …)` pair in `src/sys_util.cyr`, until cyrius ships a `sys_fcntl` wrapper (cyrius `docs/development/issues/2026-09-23-sigil-sys-fcntl-wrapper.md`).
- **No `defer` in a Result-returning fn (added 3.13.0).** cyrius 6.6.0–6.6.6 skip a `defer` block when the fn returns a value-form Result pair (`return Ok(x);`, `return Err(e);`, or a tail call to a pair-returning fn), with no diagnostic — every one of sigil's 8 `defer`s sat in such a fn and none ever ran (the LUKS key was left in `/tmp` on every format and open). Release every resource explicitly on each exit path instead. Lift this only once cyrius fixes the defect (cyrius `docs/development/issues/2026-09-22-agnodrm-defer-skipped-on-value-form-result-return.md`) AND a sigil test proves a `defer` runs on a Result return; `tests/tcyr/fd_hygiene.tcyr` is the shape of that test.
...
1. ⚠ **SUPERSEDED AS OF 3.12.3 / cycc 6.5.11 — `var X[N]` inside a function IS a per-thread stack local, and believing otherwise shipped an authentication bypass.** **cyrius 6.3.15 flipped array locals from `.bss` globals to the stack** (`_STACK_ARRAYS = 1`, cyrius `src/frontend/parse.cyr:287` at 6.3.15, `:1000` at 6.6.6), and this quirk — confirmed under **6.0.52** — was never revisited across eight releases. The banking it justified on the RSA *verify* path put **both operands of `ct_eq_bytes(rem, rexp, n_len)` in one shared lane**, so a lane collision made a forged signature verify: **888 of 400,000 accepted**, reported by agnosai. See [`docs/development/issues/archive/2026-08-08-rsa-verify-shared-lane-accepts-forged-signature.md`](docs/development/issues/archive/2026-08-08-rsa-verify-shared-lane-accepts-forged-signature.md).
   **What is true under 6.5.11**, re-probed rather than assumed: an array local **inside the per-fn stack budget of 122,880 bytes** (15,360 slots — cyrius `src/frontend/parse_decl.cyr:89` at 6.5.11, `:122` at 6.6.6; counted cumulatively over params + scalars + prior arrays) is per-call and per-thread — non-tail recursion gives four distinct frames 576 B apart, two threads land ~2 MB apart. **Past the budget it really does become a shared global**, and the compiler says so. Since cyrius **6.6.5** that is a `warning:` naming the declaration's file:line — `array local over the per-fn frame budget gets STATIC storage: one buffer shared by all calls and all threads; use alloc() for per-call storage`; before 6.6.5 it was `note: oversized array local kept in shared global (not per-thread)`, so a grep for the old text finds nothing. Treat that warning as the only reliable signal.
   ⚠ Two traps. **`CYRIUS_STACK_ARRAYS=0` reverts every array local to a shared global**, so the per-thread guarantee is one environment variable wide — never set it in a build that ships. And a probe using `return f(depth-1)` proves nothing: that is a tail call and the frame is reused, so every depth reports the same address. Use non-tail recursion.
   **Re-check every other place this quirk is invoked** (notably #9 and the `secret var` caveat above) before relying on it. Historical text follows. — confirmed by cyrius `src/frontend/parse_fn.cyr:2886` and `tests/tcyr/var_array_semantics.tcyr` (was true under cycc 6.0.52). Same-function array reuse across sequential calls works iff each call fully writes the buffer before reading; concurrent threads share the array. This used to force the `_sigil_batch_mutex` to serialise batch verify; **3.6 dropped the mutex** by giving each worker a private *bank* (lane) of every racing array via thread-local storage (`src/crypto_scratch.cyr`, cyrius 6.0.52 `lib/thread_local.cyr`) — see ADR 0001. **3.9.6: `cbank()` now AUTO-ASSIGNS a lane per thread on first use** (atomic counter → lanes `1..63`, bank 0 = main/serial), so concurrent callers — notably cyrius `tls_native`/sandhi TLS workers — get disjoint lanes with NO `crypto_bank_set` call; `SIGIL_CRYPTO_BANKS` is now 64 (every banked `var X[N]` is sized `N*64`). This closed the concurrent-TLS-handshake crash — see ADR 0007. Scalar `var x = ...` locals ARE per-call.
...
7. **Stdlib thread-safety (revised 3.13.0, re-read against the 6.6.6 snapshot)** — atomics + race-free mutex are available; `string.cyr` is safe by construction. **The allocators ARE thread-safe now:** bump `alloc()` / `alloc_reset()` take a process-wide CAS spinlock since cyrius **6.0.64** (`lib/alloc.cyr`), and `fl_alloc` / `fl_calloc` / `fl_free` / `fl_init` since **6.5.19** (`lib/freelist.cyr`; its >4096-byte path takes no lock because it touches no shared state). This quirk said "`alloc` is NOT thread-safe" from 5.5.31 until 3.13.0 — stale for most of that time. **Still NOT thread-safe: `vec` and `hashmap`** — no locking at all, so a vec or map shared across threads must not be mutated concurrently. Sigil's own lazy one-time table inits (AES, BLAKE2b, SHA-512, Ed25519) use plain flags, so the first call must still happen before workers spawn, or be pre-warmed on the main thread (`sv_verify_batch` pre-warms Ed25519 / SHA-2; `ecdsa_p256_warm` / `ecdsa_p384_warm` exist for ECDSA). ⚠ `fl_alloc`'s large path does not check its `mmap` return: on exhaustion it faults at address -12 instead of returning 0 (cyrius issue `2026-09-23-kybernet-fl-alloc-unchecked-fl-mmap-faults-at-minus-12.md`), which is why `argon2id_into` over a caller arena is the PID-1-safe entry.
